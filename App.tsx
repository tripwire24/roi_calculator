      ...
      <main className="container mx-auto px-4 sm:px-6 py-8 md:py-12">
        ...
        <div className="text-center max-w-4xl mx-auto mb-12 md:mb-16">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <button onClick={handleGetStarted} className="bg-blue-600 text-white font-bold py-3 px-8 rounded-lg shadow-lg hover:bg-blue-700 transition-transform transform hover:scale-105 w-full sm:w-auto">
              Get Started Free
            </button>
            <button onClick={handleGetStarted} className="bg-white text-slate-700 font-bold py-3 px-8 rounded-lg shadow-lg border border-slate-200 hover:bg-slate-100 transition-transform transform hover:scale-105 w-full sm:w-auto">
        ...
        {showCalculators && (
          <div id="calculator-section" className="w-full max-w-7xl mx-auto">
            {/* Tab Navigation */}
            <div className="mb-8 flex flex-wrap justify-center border-b border-slate-200">
              <button
                onClick={() => setActiveTab('ecommerce')}
                className={`px-4 sm:px-6 py-3 font-semibold text-base sm:text-lg transition-colors duration-200 ${activeTab === 'ecommerce' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-slate-500 hover:text-blue-500'}`}
              >
                E-commerce ROI
              </button>
              <button
                onClick={() => setActiveTab('leadgen')}
                className={`px-4 sm:px-6 py-3 font-semibold text-base sm:text-lg transition-colors duration-200 ${activeTab === 'leadgen' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-slate-500 hover:text-blue-500'}`}
              >
                Lead Gen ROI
              </button>
              <button
                onClick={() => setActiveTab('agency')}
                className={`px-4 sm:px-6 py-3 font-semibold text-base sm:text-lg transition-colors duration-200 ${activeTab === 'agency' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-slate-500 hover:text-blue-500'}`}
              >
                Agency Fee Audit
              </button>
            </div>
